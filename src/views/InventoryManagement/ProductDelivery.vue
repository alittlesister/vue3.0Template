    ...
    <div class="w-full mt-16 bg-white p-24" style="height: calc(100% - 126px)">
      ...
      <el-scrollbar style="width: 100%; height: calc(100% - 106px)">
        ...
        <el-table size="mini" class="bg-blue-200" :data="tableData" border style="width: 100%">
          ...
          <el-table-column prop="deliveryDate" label="出库时间"> </el-table-column>
          <el-table-column prop="deliveryPerson" label="出库人"> </el-table-column>
          <el-table-column prop="contractTotalAmount" label="合同总数量"> </el-table-column>
          <el-table-column prop="deliveryAmount" label="出库数量"> </el-table-column>
          <el-table-column prop="undeliveryAmount" label="未出库数量"> </el-table-column>

          <el-table-column width="160" label="操作">
            <template #default="scope">
              <span @click="toAddQuotation" v-if="scope" class="text-blue cursor-pointer text-sm">出库明细单</span>
              <span @click="toAddQuotation" v-if="scope" class="text-blue cursor-pointer text-sm ml-10">转发货单</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- </div> -->
  ...
  export default defineComponent({
    setup() {
      ...
      let tableData: Array<Object> = [
        {
          id: 1,
          deliveryNumber: '388632185211052735',
          purchaseNumber: '345345234',
          deliveryDate: '2004-07-16',
          deliveryPerson: '孔涛',
          contractTotalAmount: 5,
          deliveryAmount: 3424,
          undeliveryAmount: 3424
        },
        {
          id: 2,
          deliveryNumber: '388632185211052735',
      ...
      const toAddQuotation = () => {
        proxy.$store.state.tagsList.forEach((ele: any) => {
          if (ele.path.includes('/DeliveryDetails')) isInclude = true
        })
        if (!isInclude) {
        ...
        proxy.$router.push({ path: '/DeliveryDetails' })
      }
